...
class Calendar {
    ...
    beginEvent() {
        let getCurrentDate = new Date();                   // тукущая дата
        let CURRENT_DAY = getCurrentDate.getDate();              // текущий день (1...31)
        let CURRENT_MONTH = getCurrentDate.getMonth() + 1;  // текущий месяц (т.к. нумерация с 0...11, то +1)
        let CURRENT_YEAR = getCurrentDate.getFullYear();    // текущий год (2018...)

        const CURRENT_MONTH_FOR_TODAY_BUTTON = CURRENT_MONTH;

        let dayClassTitle = 'calendar__day-title';
        let monthClassTitle = 'calendar__month-title';
        let calendarClassDisabledDay = 'calendar__disabled-day';
        let calendarClassListYear = 'calendar__list-year';
        let calendarClassListMonth = 'calendar__list-month';

        /* ___main func___ */

        let table; // для построения таблици
        let currentStatus = 0; // для статуса кнопки current
        ...
        function createCalendar(id, year, month) {
            ...
            */
            ...
            let lastDays = daysInMonth(CURRENT_MONTH_FOR_TODAY_BUTTON, CURRENT_YEAR);
        ...
        function fillEmptyDaysBefore(step) {
            let lastDays = daysInMonth(CURRENT_MONTH_FOR_TODAY_BUTTON - 1, CURRENT_YEAR); // последний день предыдущего месяца
        ...
        function currentDayForBlock() {
            ...
            currentDay.innerHTML = '<div class='+ dayClassTitle +'>' + CURRENT_DAY + '</div>';
        }
        ...
        function current() {
            currentDayForBlock();
            currentMonthForBlock(CURRENT_MONTH_FOR_TODAY_BUTTON);
            createCalendar('calendar', CURRENT_YEAR, CURRENT_MONTH_FOR_TODAY_BUTTON);
